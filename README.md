# Bulk PDF Digital Signing — POC

A proof of concept for an automated, offline-capable digital signing service for B2B tax invoices, built with **FastAPI**, **pyHanko (PAdES)**, and **ReportLab**.
...
The UI is a five-step flow.

1. **Setup.** Shows the active signing certificate: the self-signed TEST Document Signer (`digitalSignature + nonRepudiation`; pyHanko requires non-repudiation). A real `.pfx` can be uploaded instead. Generates 10 uncompressed sample GST invoices (total `Rs. 59,000.00`).
2. **Bulk Sign.** Loads the `.pfx` once and signs every invoice with a PAdES signature and a visible stamp on the last page.
3. **ByteRange.** Visualises `/ByteRange` for a signed file: the two hashed ranges (green) and the signature placeholder (grey). Shows the SHA-256 of the hashed ranges next to the `messageDigest` read back out of the embedded CMS signature; they match.
4. **Tamper test.** Changes the total from `59,000.00` to `99,000.00` inside the first hashed range, then verifies both files:
   - original: document unchanged ✅, signature genuine ✅, signer trusted ✅
   - tampered: document unchanged ❌. The signature blob itself is untouched, so it is still "genuine", but it belongs to the original bytes, which no longer match.
5. **Summary.**
